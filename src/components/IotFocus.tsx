...
export default function IotFocus() {
  const [activeStep, setActiveStep] = useState<'sensor' | 'esp32' | 'websocket' | 'dashboard'>('sensor');
    ...
    <div className="iot-focus-wrapper">
      <div className="iot-intro-text">
        <p className="iot-lead">
          En tant que développeur orienté <strong>Dispositifs Interactifs & IoT</strong>, j'adore connecter 
          le monde tangible aux technologies web. Ce simulateur interactif modélise l'architecture 
          temps réel que j'ai implémentée pour lier des capteurs physiques à un dashboard de monitoring.
        </p>
      </div>
      ...
      <div className="iot-bento-grid">
        ...
        <div className="bento-card bento-simulator liquid-glass">
          ...
          <div className="simulator-controls-bar">
            ...
            <div className="slider-container">
              <input
                ...
                style={{ '--slider-fill': `${physicalPercent}%`, '--slider-accent': alertColor } as React.CSSProperties}
            ...
            <button 
              ...
              title={isAutoPlaying ? "Arrêter la simulation automatique" : "Activer la simulation automatique"}
            >
          ...
          <div className="physical-sandbox">
            ...
            <div className={`virtual-sensor ${activeStep === 'sensor' ? 'active' : ''}`} onClick={() => setActiveStep('sensor')}>
          ...
          <div className="network-architecture">
            <svg viewBox="0 0 400 120" className="architecture-svg">
              ...
              <circle r="4" fill="var(--accent-secondary)" className="data-pulse pulse-cable-1" style={{ animationDuration: `${Math.max(0.4, (simulatedDistance / 150) * 1.2)}s` }} />
              <circle r="4" fill="#0ea5e9" className="data-pulse pulse-cable-2" style={{ animationDuration: `${Math.max(0.4, (simulatedDistance / 150) * 1.2)}s` }} />
              <circle r="4" fill="var(--accent-primary)" className="data-pulse pulse-cable-3" style={{ animationDuration: `${Math.max(0.4, (simulatedDistance / 150) * 1.2)}s` }} />

              {/* Node 1: Sensor */}
              <g className={`node-g ${activeStep === 'sensor' ? 'is-active' : ''}`} onClick={() => setActiveStep('sensor')}>
                ...
                <foreignObject x="38" y="48" width="24" height="24">
                  <div className="node-icon-content"><Activity size={15} /></div>
                </foreignObject>
                <text x="50" y="95" className="node-label">Capteur</text>
              </g>

              {/* Node 2: ESP32 */}
              <g className={`node-g ${activeStep === 'esp32' ? 'is-active' : ''}`} onClick={() => setActiveStep('esp32')}>
                ...
                <foreignObject x="128" y="48" width="24" height="24">
                  <div className="node-icon-content"><Cpu size={15} /></div>
                </foreignObject>
                <text x="140" y="95" className="node-label">ESP32</text>
              </g>

              {/* Node 3: WebSocket */}
              <g className={`node-g ${activeStep === 'websocket' ? 'is-active' : ''}`} onClick={() => setActiveStep('websocket')}>
                ...
                <foreignObject x="238" y="48" width="24" height="24">
                  <div className="node-icon-content"><Layers size={15} /></div>
                </foreignObject>
                <text x="250" y="95" className="node-label">WebSockets</text>
              </g>

              {/* Node 4: Web Dashboard */}
              <g className={`node-g ${activeStep === 'dashboard' ? 'is-active' : ''}`} onClick={() => setActiveStep('dashboard')}>
                ...
                <foreignObject x="338" y="48" width="24" height="24">
                  <div className="node-icon-content"><Wifi size={15} /></div>
                </foreignObject>
                <text x="350" y="95" className="node-label">Dashboard</text>
              </g>
            </svg>
          </div>
            ...
            <div className="gauge-card">
              ...
              <div className="gauge-visual-body">
                <svg viewBox="0 0 100 100" className="radial-gauge-svg">
                  ...
                  <text x="50" y="55" className="gauge-text-val" style={{ fill: 'var(--text-primary)' }}>
                    {simulatedDistance}
                  </text>
                  <text x="50" y="70" className="gauge-text-unit" style={{ fill: 'var(--text-secondary)' }}>
                    cm
                  </text>
                </svg>
                ...
                <div className="gauge-text-details">
                  <div className="gauge-status-indicator" style={{ borderColor: alertColor }}>
                    ...
                    <span className="status-label" style={{ color: alertColor }}>
                      {simulatedDistance < 30 ? 'TROP PROCHE' : simulatedDistance < 70 ? 'MISE EN GARDE' : 'SÉCURISÉ'}
                    </span>
                  </div>
        ...
        <div className="bento-card bento-code-explorer liquid-glass">
          ...
          {/* Stepper Selector */}
          <div className="iot-stepper" role="tablist" aria-label="Étapes de transmission de données">
            {flowSteps.map((step) => (
              <button
                key={step.id}
                role="tab"
          ...
          <div className="step-details-pane">
            <div className="step-info-bubble">
              ...
              <div>
                <h4>{activeStepData.title} <span className="step-sub">{activeStepData.subtitle}</span></h4>
